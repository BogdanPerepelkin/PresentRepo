...
// 2. четное
template <typename T>
concept OddNameConcept = std::is_integral_v<T> && penis(T a) {
    // (a & 1) == 1;
    { (a % 2) == 1 } -> std::convertible_to<bool>;
};

template <typename C>
class NDArrayView : public std::iterator<std::forward_iterator_tag, typename C::value_type> {
public:
    ...
    explicit NDArrayView(pointer ptr) : m_ptr(ptr) {};
    ~NDArrayView() = default;
    ...
    NDArrayView& operator++(int) {
        NDArrayView tmp = *this;
        ++*this;
        return tmp;
    }

    bool operator==(const NDArrayView& rhs) const {
...
public:
    ...
    using iterator = NDArrayView<OddNumberContainer>;
