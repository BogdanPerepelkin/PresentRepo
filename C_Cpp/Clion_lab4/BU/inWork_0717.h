...
public:
    ...
    template<typename SizeList>
    NDArray(const SizeList &list, const T* ptr) {
        stock_init(list);
    ...
    NDArray(AbstractView<T, depth> view) {
        m_total_size = view.m_total_size;
        ...
        for (size_t i = 0; i < depth + 1; i++) {
            m_dimensions[i] = view.m_dimensions[i];
        }
        abstract::fill_accum_dim();

        create_data_by_ptr(view.m_data);
    }
    // TODO 1 - вынести небольшую копипасту
    // TODO 2 - проверить на ошибки для new (data)
    // TODO 3 - проверить конструкторы (2 штуки): по View и по 2-м View на размерность меньше
    ...
    NDArray(AbstractView<T, depth - 1> &start, AbstractView<T, depth - 1> &end) {
        ...
        for (size_t i = 0; i < depth; i++) {
            m_dimensions[i + 1] = start.m_dimensions[i];
        }
        abstract::fill_accum_dim();

        create_data_by_ptr(start.m_data);
    }
...
private:
    ...
    void stock_init(SizeList& list) {
        m_store_dims = std::array<size_type, depth + 1>();
        m_dimensions = m_store_dims.data();
