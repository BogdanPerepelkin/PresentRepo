...
public:
    ...
    NDArray(const T &scalar)
        ...
        m_accum_dim = m_store_accum.data();
        m_data = static_cast<pointer>(operator new(sizeof(T)));
        new(m_data) T(scalar);
    }
    ...
    requires(depth > 0) {
        ...
        m_data = static_cast<pointer>(operator new(sizeof(T) * m_total_size));
        pointer it = m_data;
        fill_data<depth>(init, it);
    }

    template<typename SizeList>
    NDArray(const SizeList &list, pointer ptr) {
        stock_init(list);
...
private:
    ...
    template<size_type cur_depth>
    void fill_data(typename mode_init<cur_depth, T>::type &init, pointer& it) {
        if constexpr (cur_depth > 1) {
    ...
    void create_data_by_ptr(pointer ptr) {
    ...
    void clear(pointer end = nullptr) noexcept {
