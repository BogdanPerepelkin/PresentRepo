...
public:
    ...
    NextConstView begin() const noexcept requires(depth > 0) {
        return NextConstView(m_data, m_dimensions + 1, m_accum_dim + 1);
    }
    T* begin() noexcept requires(depth == 0) {
        return m_data;
    }
    const T* begin() const noexcept requires(depth == 0) {
        return m_data;
    }
    ...
    NextConstView end() const noexcept requires(depth > 0) {
    ...
    const T* end() const noexcept requires(depth == 0) {
        return m_data + m_dimensions[0] * m_accum_dim[0];
    }
    ...
    T* cbegin() const noexcept requires(depth == 1) {
        return m_data;
    }
    ...
    T* cend() const noexcept requires(depth == 1) {
...
// Constructors:
    // NDArray(const T* impr_data, const size_t* impr_dimensions, const size_t* impr_accum_dim, const size_t impr_total_size) {
    ...
    NDArray(const NDArray& other) {
        ...
        // 3. Безопасно копируем элементы с ручным управлением жизненным циклом
        size_t i = 0;
        try {
            ...
            // Уничтожаем уже сконструированные
    ...
    NDArray(const T& scalar) requires (depth == 0) {
        m_total_size = 1;
        m_dimensions = new size_t[1]{1};
        m_accum_dim = new size_t[1]{1};
        // m_data = std::allocator<T>{}.allocate(m_total_size);
        m_data = static_cast<T*>(::operator new(sizeof(T) * m_total_size));
        *m_data = scalar;
    }
