...
public:
    ...
    NextView operator*() const requires(depth > 0) {
...
// Constructors:
    ...
    // }
    // view end() noexcept {
        // return NDArrayView<T, depth - 1>(m_data + m_total_size, m_dimensions, m_accum_dim).begin();
    // }
    ThisView begin() noexcept requires(depth > 1){
        std::cout << "depth: " << depth << "\n";
        return ThisView(m_data, m_dimensions, m_accum_dim ).begin();
    }
    ThisView end() noexcept requires(depth > 1){
